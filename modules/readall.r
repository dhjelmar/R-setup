readall <- function(datasource,
                    ...
                    na=c(NA, 'NA', 'na', 'N/A', 'n/a', 'NaN', 'nan', ''), # strings to read as NA rather than character
    ...
    if (ext == 'csv' | ext == 'txt') {
        if (header.row == 0) {
            ...
            ## read header
            hd <- read.table(datasource, header=TRUE,  sep=",", skip=header.row-1, nrows=1, stringsAsFactors=FALSE, na.strings=na, check.names=FALSE)
            ## read data
            df <- read.table(datasource, header=FALSE, sep=",", skip=data.start.row-1, stringsAsFactors=FALSE, na.strings=na)
        ...
        ## assume data source is an internal table
...
readall_test <- function() {
  ...
  ## use above info to identify this as data table rather than external file
   ...
   2 	75 	NA 	na 	1580 	29 	26 
   3 	55 	25 	5 	NaN 	n/a	20
   "
