readall <- function(datasource,
    ...
    if (lastone == '\n') {
        ...
        if (header.row == 0) {
            ...
            ## read header
            hd <- read.table(datasource, header=TRUE,  sep=",", skip=header.row-1,    nrows=nheader)
            ## read data
            df <- read.table(datasource, header=FALSE, sep=",", skip=data.start.row-1)
        ...
        if (!is.null(range)) {
            ...
            if (header.row !=0) {
                ...
                ## no header provided
                df <- read_excel(datasource, sheet=sheet, col_names=FALSE, range=range)
            }
            ...
            if (header.row == 0) {
                ...
                if (is.null(n_max)) {
                    ## read to end of file
                    df <- read_excel(datasource, sheet=sheet, col_names=FALSE, skip=data.start.row-1)
                } else {
                ...
                if (is.null(n_max)) {
                    ## read to end of file
                    df <- read_excel(datasource, sheet=sheet, col_names=TRUE, skip=header.row-1)
                } else {
                    ## add 1 to n_max to read header plus specified number of data rows
                    df <- read_excel(datasource, sheet=sheet, col_names=TRUE, skip=header.row-1, n_max=n_max+1)
                }
            } else {
                ## lines between header row and data
                ## read header
                hd <- read_excel(datasource, sheet=sheet, col_names=TRUE,  skip=header.row-1, n_max=1)
                if (is.null(n_max)) {
